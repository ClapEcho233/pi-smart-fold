...
- **点击展开**：点击折叠行 → pi 原生切换为 `Thinking...`；再点一次 → 展开**完整思考全文**，并在思考内容**底部**保留加粗的 `Thought for 12.4s` 时长行（与斜体思考正文风格稍作区分）。
...
其它模式：`tail`（始终单行显示结尾 + 加粗时长前缀）、`full`（结束后直接显示全文，底部同样附时长行）、`off`（关闭）。
